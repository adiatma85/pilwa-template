...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.suara.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.suaras.update", [$suara->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('calon') ? 'is-invalid' : '' }}" name="calon_id" id="calon_id">
                    @foreach($calons as $id => $entry)
                        <option value="{{ $id }}" {{ (old('calon_id') ? old('calon_id') : $suara->calon->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('calon'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('peserta') ? 'is-invalid' : '' }}" name="peserta_id" id="peserta_id">
                    @foreach($pesertas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('peserta_id') ? old('peserta_id') : $suara->peserta->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type">
                    ...
                    @foreach(App\Models\Suara::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', $suara->type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
